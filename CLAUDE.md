...
- **BUG (data-kwaliteit, niet acuut): AHK-logformaat sinds 2026-07-16 wordt onjuist geparsed.** De AHK-tracker is per 2026-07-16 overgestapt op een nieuw regelformaat (extra velden `process` en `detail`, plus `MARK`-regels voor handmatige projecttoewijzing) — midden in het lopende logbestand. De huidige `importer.py`/`LINE_RE` crasht hier niet op en blokkeert geen import, maar parsed nieuwe-formaat-regels stilzwijgend fout: `raw_title` wordt `process | detail | title` aan elkaar geplakt, waardoor `detect_noise()` `Idle`/`Desktop`/`Untitled`-regels niet meer herkent (idle tijd telt ten onrechte mee als actief) en `extract_app_name()` soms de verkeerde appnaam afleidt. `MARK`-regels matchen de regex helemaal niet en worden stilletjes (zonder log-waarschuwing) genegeerd. Concreet gevolg: gedeclareerde/actieve tijd sinds 2026-07-16 is mogelijk onbetrouwbaar totdat dit gerepareerd is. Volledige implementatiespec (per-regel formaatdetectie, MARK-scope, tests) staat in [docs/ahk-log-format-v2-importer-update.md](docs/ahk-log-format-v2-importer-update.md).
- **Interface: extra gelogde info (process/detail) gebruiken voor snellere projecttoewijzing.** Het nieuwe logformaat (zie hierboven) levert per activiteit ook het procesnaam- en detailveld (URL, bestandspad, mail-onderwerp, afspraakonderwerp) mee. Zodra de importer dit correct opslaat, kan de UI dit tonen in de toewijzingspopup/tooltip (bijv. bestandspad of URL naast de titel) zodat in één oogopslag duidelijk is bij welk project een blok hoort — in plaats van alleen de vaak generieke venstertitel. Zie ook de aanbeveling in [docs/ahk-log-format-v2-importer-update.md](docs/ahk-log-format-v2-importer-update.md) om regels bij voorkeur op het detail-veld te laten matchen in plaats van op de titel.
...
- **Weekstaat vergrendelen en/of sync beperken tot huidige week.** Twee samenhangende problemen: (1) een afgeronde weekstaat kan per ongeluk worden overschreven door een sync die ook oudere dagen herberekent; (2) de sync verwerkt nu het volledige logbestand (alle aanwezige dagen), terwijl de gebruiker bij dagelijks gebruik alleen de lopende week wil bijwerken.

  **Optie A — Sync beperken tot geselecteerde week (snelle winst):** Voeg aan `POST /api/activities/sync/` parameters `date_from` en `date_to` toe. De frontend sync-knop stuurt de ISO-weekgrenzen mee van de week die op dat moment geselecteerd is in het dashboard/de weekstaat (`currentDate` in de Pinia store bepaalt deze week). Zo synct een klik op de sync-knop altijd de week die de gebruiker op dat moment bekijkt, niet per se de kalenderweek van vandaag. Een expliciete "alles herberekenen"-actie (bijv. in ⚙️) laat `date_from`/`date_to` weg en verwerkt het volledige logbestand.

  **Optie B — Weekstaat vergrendelen (structurele bescherming):** Voeg een `WeekLock`-model toe (`year`, `week`, `locked_at`, `locked_by`). Een vergrendelde week wordt overgeslagen door `aggregate_day()` en de sync-endpoint (tenzij `force=true` wordt meegegeven). De Weekstaat-pagina toont een slotknop per week; vergrendelde weken zijn read-only in het dashboard (blokken kunnen niet worden verplaatst/toegewezen). Backend: nieuw model + migratie + check in aggregator. Frontend: slotknop in Weekstaat, visuele read-only-staat in dashboard.

  Beide opties kunnen onafhankelijk worden geïmplementeerd; Optie A is de snellere fix, Optie B de robuuste langetermijnoplossing. Samen oppakken met "Commentaar per weekstaat" en "Projectnummers" zodat het `WeekLock`-model direct ook een commentaar-veld krijgt.
