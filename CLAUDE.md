...
- **BUG: Verkleind blok laat vrijgekomen slot niet zien als unassigned.** Als twee aaneengesloten blokken aan hetzelfde project zijn toegewezen en het onderste blok wordt verkleind door de onderste handle omhoog te slepen, verschijnt het vrijgekomen 15-min slot niet als lichtgrijs unassigned blok met hover-activiteiten. Na sync klikken wordt het slot wél correct getoond. Vermoedelijke oorzaak: de frontend past `mergedBlocksByDay` of de lokale store-state niet opnieuw aan na resize; het vrijgekomen slot ontbreekt in de store totdat een volledige herlaad plaatsvindt.
- **BUG: Ontkoppeld blok wordt na sync opnieuw toegewezen.** Als via de popup "koppeling verwijderen" wordt geklikt (unassign) en daarna sync wordt uitgevoerd, krijgt het blok zijn project opnieuw toegewezen. Vermoedelijke oorzaak: de sync herberekent de dag en past de rule engine toe, maar de handmatige unassign wordt niet als `assigned_by='manual'` met `project_id=null` in `BlockProjectHistory` opgeslagen — waardoor de rule engine het blok alsnog toewijst. Oplossing: zorg dat unassign ook een `BlockProjectHistory`-record aanmaakt (met `project=null` of een expliciete `unassigned`-markering) die de rule engine respecteert.
